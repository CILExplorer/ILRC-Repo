...
import CaseNoteEditorClient from './CaseNoteEditorClient';

export const revalidate = 0;

type Params = Promise<{ id: string }>;

export default async function AdminCaseNoteEditorPage({ params }: { params: Params }) {
  // 1. Authenticate
  ...
  // 2. Fetch the case note from DB
  let caseNote = null;
  try {
    const result = await query('SELECT * FROM case_notes WHERE id = $1', [id]);
    if (result.rows.length > 0) {
      caseNote = result.rows[0];
    }
  } catch (error) {
    console.error('Error fetching case note for editing:', error);
  }

  if (!caseNote) {
    return (
      <div style={{ padding: '2rem', textAlign: 'center' }}>
        <h2>Case Note Not Found</h2>
  ...
  return (
    <div className="admin-layout" style={{ minHeight: '100vh', display: 'flex' }}>
      ...
      <main className="admin-main">
        <CaseNoteEditorClient caseNote={caseNote} />
      </main>
    </div>
  );
}
